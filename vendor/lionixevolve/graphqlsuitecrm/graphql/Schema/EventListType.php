...
class EventListType extends AbstractListType
{
    ...
    {
        foreach (argsHelper::entityArgsHelper('FP_events', true) as $field => $type) {
    ...
    public function resolve($value = null, $args = [], $info = null)
    {
        ...
        $list=ListHelper('FP_events', $value, $args, $info);
        $resultArray = [];
        if (is_array($list['list']) && !empty($list['list'])) {
            if ($list['list'][0]->ACLAccess('list')) {
                foreach ($list['list'] as $item) {
                    $resultArray[] = EventType::resolve(null, ['id' => $item->id], $info);
                }
            } else {
                //no access
                error_log('no access');
