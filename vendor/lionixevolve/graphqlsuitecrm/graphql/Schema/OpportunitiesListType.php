...
class OpportunitiesListType extends AbstractListType
{
    ...
    public function resolve($value = null, $args = [], $info = null)
    {
        ...
        $list=ListHelper('Opportunities', $value, $args, $info);
        $resultArray = [];

        if (is_array($list['list']) && !empty($list['list'])) {
            if ($list['list'][0]->ACLAccess('list')) {
                foreach ($list['list'] as $item) {
                    $resultArray[] = OpportunityType::resolve(null, ['id' => $item->id], $info);
                }
            } else {
                //no access
                error_log('no access');
