...
class SuiteCRMSchema extends AbstractSchema
{
    public function build(SchemaConfig $config)
    {
        $config->getQuery()->addFields([
          ...
          'samples'           => [
                        'type' => new SampleListType(),
                        'args'    => argsHelper::entityArgsHelper('Thin_Sample_Lot'),
                        'resolve'    => function ($value, array $args, ResolveInfo $info) {
                            return $info->getReturnType()->resolve($value, $args, $info);
                        }
                    ],
            'thin_cycleplantargets'           => [
               ...
               'events'           => [
                'type' => new EventListType(),
                'args'    => argsHelper::entityArgsHelper('FP_events'),
                'resolve'    => function ($value, array $args, ResolveInfo $info) {
                    return $info->getReturnType()->resolve($value, $args, $info);
                }
            ],
        ...
        $config->getMutation()->addFields([
            'createAccount' => [
             ...
             'createProduct' => [
                 'type' => new ProductInputType(),
                 'args'    => array_merge(argsHelper::entityArgsHelper('AOS_Products', true),['related_beans' => new ListType(new RelatedBeanInputType())]),
                 'resolve' => function ($value, $args, $type) {
                     $result=ProductInputType::resolve($value, $args, $type);
                        return ProductType::resolve($value, ['id'=>$result['id']], $type);
                 },
             ],
             'createEvent' => [
                 'type' => new EventInputType(),
                 'args'    => array_merge(argsHelper::entityArgsHelper('FP_events', true),['related_beans' => new ListType(new RelatedBeanInputType())]),
                 'resolve' => function ($value, $args, $type) {
                     $result=EventInputType::resolve($value, $args, $type);
                        return EventType::resolve($value, ['id'=>$result['id']], $type);
                 },
             ],
        ]);
