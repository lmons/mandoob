        ...
        if ($call->id && $call->ACLAccess('view')) {
            ...
            switch ($module_arr['parent_type']) {
                ...
                if(isset($queryFields) && array_key_exists('aos_products',$queryFields)){
                 $module_arr['products'] =  array();
                 foreach ($call->get_linked_beans('calls_aos_products_1') as $product) {
                     $module_arr['products'][] = $product->id;
                 }
             }
              if(isset($queryFields) && array_key_exists('samples',$queryFields)){
                 $module_arr['samples'] =  array();
                 foreach ($call->get_linked_beans('calls_thin_sample_lot_1') as $sample) {
                     $module_arr['samples'][] = $sample->id;
                 }
             }
              if(isset($queryFields) && array_key_exists('key_messages',$queryFields)){
                 $module_arr['key_messages'] =  array();
                 foreach ($call->get_linked_beans('calls_thin_key_messages_1') as $keymessage) {
