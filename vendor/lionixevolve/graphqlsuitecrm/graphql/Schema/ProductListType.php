...
class ProductListType extends AbstractListType
{
    ...
    {
        foreach (argsHelper::entityArgsHelper('AOS_Products', true) as $field => $type) {
    ...
    public function resolve($value = null, $args = [], $info = null)
    {
        ...
        $list=ListHelper('AOS_Products', $value, $args, $info);
        $resultArray = [];

        if (is_array($list['list']) && !empty($list['list'])) {
            if ($list['list'][0]->ACLAccess('list')) {
                foreach ($list['list'] as $item) {
                    $resultArray[] = ProductType::resolve(null, ['id' => $item->id], $info);
                }
            } else {
                //no access
                error_log('no access');
