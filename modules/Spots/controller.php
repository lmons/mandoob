...
FROM
  ...
  INNER JOIN calls ON calls.parent_id = accounts.id
  INNER JOIN users ON users.id = calls.assigned_user_id
  INNER JOIN securitygroups_users ON users.id = securitygroups_users.user_id
  INNER JOIN securitygroups ON securitygroups.id = securitygroups_users.securitygroup_id
  WHERE calls.status='Held' AND calls.assigned_user_id=cp.assigned_user_id AND calls.date_start BETWEEN CONCAT(cp.start_date,' 00:00') AND CONCAT(cp.end_date,' 23:59') 
    ORDER BY calls.date_start,users.last_name ASC 
";    
    $mysqlQueryCalls = <<<EOF
        SELECT
            'call' as type
...
EOF;
        ...
        while ($row = $db->fetchByAssoc($result)) {
            $x = new stdClass();
            $x->{$mod_strings['LBL_AN_ACCOUNTS_PHARMA_DAY']} = date("d/m/y", strtotime($row['date_start']));
            $x->{$mod_strings['LBL_AN_ACCOUNTS_PHARMA_MONTH']} = date("Y", strtotime($row['date_start']))."-".$app_list_strings["dom_cal_month_long"][date("n", strtotime($row['date_start']))];
            $x->{$mod_strings['LBL_AN_ACCOUNTS_PHARMA_ACCOUNT_NAME']} = $row['name1'];
            $x->{$mod_strings['LBL_AN_ACCOUNTS_PHARMA_CUTOMER_TIER']} = $row['customer_tier'];
            $x->{$mod_strings['LBL_AN_ACCOUNTS_PHARMA_ACCOUNT_TYPE']} = $app_list_strings["account_type_dom"][$row['account_type']];
            $x->{$mod_strings['LBL_AN_ACCOUNTS_PHARMA_REP']} = $row['user_first_name']." ".$row['user_last_name'];
            $x->{$mod_strings['LBL_AN_ACCOUNTS_PHARMA_WEEK_C']} = $row['week_c'];
            $x->{$mod_strings['LBL_AN_ACCOUNTS_PHARMA_TEAM']} = $row['name2'];
            $returnArray[] = $x;
        }
...
FROM
  ...
  INNER JOIN calls_contacts ON calls_contacts.contact_id = contacts.id
  ...
  INNER JOIN securitygroups ON securitygroups_users.securitygroup_id = securitygroups.id
    WHERE calls.status='Held' AND calls.assigned_user_id=cp.assigned_user_id AND calls.date_start BETWEEN CONCAT(cp.start_date,' 00:00') AND CONCAT(cp.end_date,' 23:59')
    ORDER BY cp.start_date,users.last_name ASC 
";
        ...
        while ($row = $db->fetchByAssoc($result)) {
            ...
            $x->{$mod_strings['LBL_AN_LEADS_WEEK']} = $row['week_c'];
...
FROM
  ...
  WHERE ".$custom_where." contacts.id NOT IN (SELECT calls.parent_id FROM calls WHERE calls.assigned_user_id=cp.assigned_user_id AND calls.date_start BETWEEN CONCAT(cp.start_date,' 00:00') AND CONCAT(cp.end_date,' 23:59') )
  ORDER BY
  cp.start_date,users.last_name ASC
";
        ...
        while ($row = $db->fetchByAssoc($result)) {
            $x = new stdClass();
            $x->{$mod_strings['LBL_AN_ACTIVITIES_DAY']} = date("d/m/y", strtotime($row['start_date']));
            $x->{$mod_strings['LBL_AN_ACTIVITIES_MONTH']} = date("Y", strtotime($row['date_start']))."-".$app_list_strings["dom_cal_month_long"][date("n", strtotime($row['date_start']))];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_CONTACT_NAME']} = $row['first_name']." ".$row['last_name'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_CUTOMER_TIER']} = $row['customer_tier'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_SPECIALTY_C']} = $app_list_strings["thin_specialty_list"][$row['specialty_c']];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_REP']} = $row['user_first_name']." ".$row['user_last_name'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_WEEK_C']} = $row['week_c'];
            $returnArray[] = $x;
        }
...
FROM
  ...
  INNER JOIN thin_cycleplantargets_contacts_c AS cpt_contacts ON
    cpts.id_c = cpt_contacts.thin_cycleplantargets_contactsthin_cycleplantargets_idb
  INNER JOIN accounts ON accounts.id = cpt_contacts.thin_cycleplantargets_contactscontacts_ida
  INNER JOIN calls ON calls.parent_id = accounts.id
  INNER JOIN users ON users.id = calls.assigned_user_id
  WHERE calls.status='Held' AND cp.assigned_user_id IN ($xc'')  AND calls.assigned_user_id=cp.assigned_user_id AND calls.date_start BETWEEN CONCAT(cp.start_date,' 00:00') AND CONCAT(cp.end_date,' 23:59') 
    ORDER BY calls.date_start,users.last_name ASC 
";    
    $myOwnSqlQuery="
   SELECT
...
EOF;
        ...
        while ($row = $db->fetchByAssoc($result)) {
            $x = new stdClass();
            $x->{$mod_strings['LBL_AN_ACTIVITIES_DAY']} = date("d/m/y", strtotime($row['date_start']));
            $x->{$mod_strings['LBL_AN_ACTIVITIES_MONTH']} = date("Y", strtotime($row['date_start']))."-".$app_list_strings["dom_cal_month_long"][date("n", strtotime($row['date_start']))];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_CONTACT_NAME']} = $row['first_name']." ".$row['last_name'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_CUTOMER_TIER']} = $row['customer_tier'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_SPECIALTY_C']} = $app_list_strings["thin_specialty_list"][$row['specialty_c']];
  ...
  /*      usort($returnArray, function($a, $b) {
    return strtotime($a[$mod_strings['LBL_AN_ACTIVITIES_TYPE']]) - strtotime($b[$mod_strings['LBL_AN_ACTIVITIES_TYPE']]);
       ...
       public function action_createActivitiesPharmaSpotsData($filepath)
    {
        global $mod_strings,$current_user,$app_list_strings;
        $returnArray = array();
        $db = DBManagerFactory::getInstance();
...
EOF;
        ...
        while ($row = $db->fetchByAssoc($result)) {
            ...
            $x->{$mod_strings['LBL_AN_ACTIVITIES_DAY']} = date("d/m/y", strtotime($row['date_start']));
            $x->{$mod_strings['LBL_AN_ACTIVITIES_MONTH']} = date("Y", strtotime($row['date_start']))."-".$app_list_strings["dom_cal_month_long"][date("n", strtotime($row['date_start']))];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_CONTACT_NAME']} = $row['name1'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_CUTOMER_TIER']} = $row['customer_tier'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_SPECIALTY_C']} =$app_list_strings["account_type_dom"][$row['account_type']];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_REP']} = $row['user_first_name']." ".$row['user_last_name'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_WEEK_C']} = $row['week_c'];
            $returnArray[] = $x;
        }
...
FROM
  ...
  INNER JOIN accounts_thin_cycleplantargets_1_c ON
    accounts_thin_cycleplantargets_1_c.accounts_thin_cycleplantargets_1thin_cycleplantargets_idb = cpts.id_c
  INNER JOIN accounts ON accounts_thin_cycleplantargets_1_c.accounts_thin_cycleplantargets_1accounts_ida = accounts.id
  INNER JOIN calls ON calls.parent_id = accounts.id
  INNER JOIN users ON users.id = calls.assigned_user_id
  WHERE  calls.status='Held' AND cp.assigned_user_id IN ($xc'')  AND calls.assigned_user_id=cp.assigned_user_id AND calls.date_start BETWEEN CONCAT(cp.start_date,' 00:00') AND CONCAT(cp.end_date,' 23:59')
    ORDER BY calls.date_start,users.last_name ASC 
...
EOF;
        ...
        while ($row = $db->fetchByAssoc($result)) {
            $x = new stdClass();
            $x->{$mod_strings['LBL_AN_MARKETING_PHARMA_DAY']} = date("d/m/y", strtotime($row['date_start']));
            $x->{$mod_strings['LBL_AN_MARKETING_PHARMA_MONTH']} = date("Y", strtotime($row['date_start']))."-".$app_list_strings["dom_cal_month_long"][date("n", strtotime($row['date_start']))];
            $x->{$mod_strings['LBL_AN_MARKETING_PHARMA_NAME']} = $row['name1'];
            $x->{$mod_strings['LBL_AN_MARKETING_PHARMA_CUTOMER_TIER']} = $row['customer_tier'];
            $x->{$mod_strings['LBL_AN_MARKETING_PHARMA_ACCOUNT_TYPE']} = $app_list_strings["account_type_dom"][$row['account_type']];
            $x->{$mod_strings['LBL_AN_MARKETING_PHARMA_REP']} = $row['user_first_name']." ".$row['user_last_name'];
            $x->{$mod_strings['LBL_AN_MARKETING_PHARMA_WEEK_C']} = $row['week_c'];
            /*   $x->{$mod_strings['LBL_AN_MARKETING_BUDGET']} = date("d/m/y", strtotime($row['date_start']));
            $x->{$mod_strings['LBL_AN_MARKETING_EXPECTED_COST']} = date("Y", strtotime($row['date_start']))."-".date("F", strtotime($row['date_start']));
            $x->{$mod_strings['LBL_AN_MARKETING_EXPECTED_REVENUE']} = $row['name1'];
            $x->{$mod_strings['LBL_AN_MARKETING_OPPORTUNITY_NAME']} = $row['customer_tier'];
            $x->{$mod_strings['LBL_AN_MARKETING_OPPORTUNITY_AMOUNT']} =$row['account_type'];
            $x->{$mod_strings['LBL_AN_MARKETING_OPPORTUNITY_SALES_STAGE']} = $row['user_first_name']." ".$row['user_last_name'];
            $x->{$mod_strings['LBL_AN_MARKETING_OPPORTUNITY_ASSIGNED_TO']} =$row['week_c'];*/
            $returnArray[] = $x;
        }
        file_put_contents($filepath, json_encode($returnArray));
    }
