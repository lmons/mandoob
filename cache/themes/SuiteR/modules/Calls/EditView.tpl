...
<select name='parent_type' tabindex="0" id='parent_type' title=''   accesskey='7' onchange='document.{$form_name}.{$fields.parent_name.name}.value="";document.{$form_name}.parent_id.value=""; changeParentQS("{$fields.parent_name.name}"); checkParentType(document.{$form_name}.parent_type.value, document.{$form_name}.btn_{$fields.parent_name.name});'>
...
{capture name="label" assign="label"}{sugar_translate label='LBL_CALL_LOCATION' module='Calls'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.call_location_c.name}" 
id="{$fields.call_location_c.name}" 
title=''       
>
{if isset($fields.call_location_c.value) && $fields.call_location_c.value != ''}
{html_options options=$fields.call_location_c.options selected=$fields.call_location_c.value}
{else}
{html_options options=$fields.call_location_c.options selected=$fields.call_location_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.call_location_c.options }
{capture name="field_val"}{$fields.call_location_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.call_location_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.call_location_c.name}" 
id="{$fields.call_location_c.name}" 
title=''          
>
{if isset($fields.call_location_c.value) && $fields.call_location_c.value != ''}
...
<input
id="{$fields.call_location_c.name}-input"
name="{$fields.call_location_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.call_location_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.call_location_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.call_location_c.name}-input', '{$fields.call_location_c.name}');sync_{$fields.call_location_c.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
	SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.call_location_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.call_location_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.call_location_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.call_location_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.call_location_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.call_location_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.call_location_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
<input type="text" name="{$fields.accompanying_c.name}" class="sqsEnabled" tabindex="0" id="{$fields.accompanying_c.name}" size="" value="{$fields.accompanying_c.value}" title='' autocomplete="off" READONLY 	 >
<input type="hidden" name="{$fields.accompanying_c.id_name}" 
id="{$fields.accompanying_c.id_name}" 
value="{$fields.user_id_c.value}">
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.accompanying_c.name}" id="btn_{$fields.accompanying_c.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_SELECT_USERS_TITLE"}" class="button firstChild" value="{sugar_translate label="LBL_ACCESSKEY_SELECT_USERS_LABEL"}"
onclick='open_popup(
"{$fields.accompanying_c.module}", 
600, 
400, 
"", 
true, 
false, 
{literal}{"call_back_function":"set_return","form_name":"EditView","field_to_name_array":{"id":"user_id_c","name":"accompanying_c"}}{/literal}, 
"single", 
true
);' ><img src="{sugar_getimagepath file="id-ff-select.png"}"></button><button type="button" name="btn_clr_{$fields.accompanying_c.name}" id="btn_clr_{$fields.accompanying_c.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_CLEAR_USERS_TITLE"}"  class="button lastChild"
onclick="SUGAR.clearRelateField(this.form, '{$fields.accompanying_c.name}', '{$fields.accompanying_c.id_name}');"  value="{sugar_translate label="LBL_ACCESSKEY_CLEAR_USERS_LABEL"}" ><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
<script type="text/javascript">
SUGAR.util.doWhen(
		"typeof(sqs_objects) != 'undefined' && typeof(sqs_objects['{$form_name}_{$fields.accompanying_c.name}']) != 'undefined'",
		enableQS
);
</script>
</tr>
...
<td valign="top" width='37.5%' data-total-columns="2" >
{counter name="panelFieldCount"}
{include file="custom/modules/Calls/tpls/productOrder.tpl"}
<td valign="top" id='_label' width='12.5%' data-total-columns="2" scope="col">
&nbsp;
</td>
{counter name="fieldsUsed"}
...
<td valign="top" width='37.5%' data-total-columns="2" >
{counter name="panelFieldCount"}
{include file="custom/modules/Calls/tpls/products.tpl"}
</tr>
{/capture}
{if $fieldsUsed > 0 }
{$tableRow}
...
{sugar_translate label='LBL_EDITVIEW_PANEL2' module='Calls'}
<script>
...
<td valign="top" width='37.5%' data-total-columns="1" colspan='3'>
{counter name="panelFieldCount"}
{include file="custom/modules/Calls/tpls/samples.tpl"}
</tr>
{/capture}
{if $fieldsUsed > 0 }
{$tableRow}
...
{sugar_translate label='LBL_EDITVIEW_PANEL3' module='Calls'}
<script>
...
<td valign="top" width='37.5%' data-total-columns="1" colspan='3'>
{counter name="panelFieldCount"}
{include file="custom/modules/Calls/tpls/keyMessages.tpl"}
</tr>
{/capture}
{if $fieldsUsed > 0 }
{$tableRow}
...
{sugar_translate label='CONATCTS' module='Calls'}
<script>
...
<td valign="top" width='37.5%' data-total-columns="1" colspan='3'>
{counter name="panelFieldCount"}
{include file="custom/modules/Calls/tpls/contactsInAccount.tpl"}
</tr>
{/capture}
{if $fieldsUsed > 0 }
{$tableRow}
...
addToValidate('EditView', 'parent_name', 'parent', true,'{/literal}{sugar_translate label='LBL_LIST_RELATED_TO' module='Calls' for_js=true}{literal}' );
...
addToValidate('EditView', 'call_type_c', 'enum', false,'{/literal}{sugar_translate label='LBL_CALL_TYPE' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'accompanying_c', 'relate', false,'{/literal}{sugar_translate label='LBL_ACCOMPANYING' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'call_location_c', 'enum', false,'{/literal}{sugar_translate label='LBL_CALL_LOCATION' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'user_id_c', 'id', false,'{/literal}{sugar_translate label='LBL_ACCOMPANYING_USER_ID' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'call_location_name_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_CALL_LOCATION_NAME' module='Calls' for_js=true}{literal}' );
...
addToValidateBinaryDependency('EditView', 'accompanying_c', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='Calls' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_ACCOMPANYING' module='Calls' for_js=true}{literal}', 'user_id_c' );
