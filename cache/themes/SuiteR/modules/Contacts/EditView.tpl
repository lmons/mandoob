...
{capture name="label" assign="label"}{sugar_translate label='LBL_PRIMARY_ADDRESS_CITY' module='Contacts'}{/capture}
...
{capture name="label" assign="label"}{sugar_translate label='LBL_SPECIALTY' module='Contacts'}{/capture}
{$label|strip_semicolon}:
<span class="required">*</span>
</td>
{counter name="fieldsUsed"}
...
<select name="{$fields.specialty_c.name}" 
id="{$fields.specialty_c.name}" 
title=''       
>
{if isset($fields.specialty_c.value) && $fields.specialty_c.value != ''}
{html_options options=$fields.specialty_c.options selected=$fields.specialty_c.value}
{else}
{html_options options=$fields.specialty_c.options selected=$fields.specialty_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.specialty_c.options }
{capture name="field_val"}{$fields.specialty_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.specialty_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.specialty_c.name}" 
id="{$fields.specialty_c.name}" 
title=''          
>
{if isset($fields.specialty_c.value) && $fields.specialty_c.value != ''}
...
<input
id="{$fields.specialty_c.name}-input"
name="{$fields.specialty_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.specialty_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.specialty_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.specialty_c.name}-input', '{$fields.specialty_c.name}');sync_{$fields.specialty_c.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
	...
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.specialty_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.specialty_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.specialty_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.specialty_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.specialty_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.specialty_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.specialty_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
{capture name="label" assign="label"}{sugar_translate label='LBL_PRC_NUMBER' module='Contacts'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
addToValidate('EditView', 'city_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_CITY' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'kol_c', 'bool', false,'{/literal}{sugar_translate label='LBL_KOL' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'prc_number_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_PRC_NUMBER' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'address_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_ADDRESS' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'gender_c', 'enum', false,'{/literal}{sugar_translate label='LBL_GENDER' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'specialty_c', 'enum', true,'{/literal}{sugar_translate label='LBL_SPECIALTY' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'type_c', 'enum', false,'{/literal}{sugar_translate label='LBL_TYPE' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'brick_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_BRICK' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'kol_type_c', 'enum', false,'{/literal}{sugar_translate label='LBL_KOL_TYPE' module='Contacts' for_js=true}{literal}' );
