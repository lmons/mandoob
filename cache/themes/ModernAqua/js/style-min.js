...
setTimeout("window.print();",1000);});YAHOO.util.Event.onDOMReady(function(){var module=(tmp=document.location.search.match(/module=([a-zA-Z0-9_]*)/))?tmp[1]:'';var action=(tmp=document.location.search.match(/action=([a-zA-Z0-9_]*)/))?tmp[1]:'index';if(module=='')module=typeof(module_sugar_grp1)!='undefined'?module_sugar_grp1:'Home';document.body.id=module;document.body.className+=(document.body.className)?' '+action:action;});YAHOO.util.Event.onAvailable('sitemapLinkSpan',function(){document.getElementById('sitemapLinkSpan').onclick=function(){ajaxStatus.showStatus(SUGAR.language.get('app_strings','LBL_LOADING_PAGE'));var smMarkup='';var callback={success:function(r){ajaxStatus.hideStatus();document.getElementById('sm_holder').innerHTML=r.responseText;with(document.getElementById('sitemap').style){display="block";position="fixed";right=0;top=80;}
document.getElementById('sitemapClose').onclick=function(){document.getElementById('sitemap').style.display="none";}}}
