 ...
 ********************************************************************************/
    console.log('custom calendar js file')
var CAL = {};
CAL.slot_height = 14;
CAL.dropped = 0;
...
CAL.dialog_save = function () {
    ...
    if ((CAL.get("radio_meeting").checked == true) && CAL.get("tm_type_c")!==null && (CAL.get("tm_type_c").value == "")) {
      CAL.enable_buttons();
        CAL.get("tm_type_c").style.background = "red";
            setTimeout(function background() {
                CAL.get("tm_type_c").style.background = "white";
            },2000);
     CAL.get("title-cal-edit").innerHTML = "Please enter a Type.";
      return;
  }
  if ((CAL.get("radio_meeting").checked == true) && CAL.get("duration").value == "") {
     CAL.enable_buttons();
     CAL.get("duration").style.background = "red";
            setTimeout(function background() {
                CAL.get("duration").style.background = "white";
            },2000);
      CAL.get("title-cal-edit").innerHTML = "date is required";
      return;
  }
  if ((CAL.get("radio_call").checked == true) && CAL.get("parent_name").value == "") {
     CAL.enable_buttons();
     CAL.get("parent_name").style.background = "red";
            setTimeout(function background() {
                CAL.get("parent_name").style.background = "white";
            },2000);
      CAL.get("title-cal-edit").innerHTML = "name is required";
      return;
  } 
  
    if (CAL.get("date_start_date").value == "") {
     CAL.enable_buttons();
     CAL.get("date_start_date").style.background = "red";
            setTimeout(function background() {
                CAL.get("date_start_date").style.background = "white";
            },2000);
      CAL.get("title-cal-edit").innerHTML = "date is required";
      return;
  } 
     if (CAL.get("duration_hours").value == "") {
     CAL.enable_buttons();
     CAL.get("duration_hours").style.background = "red";
            setTimeout(function background() {
                CAL.get("duration_hours").style.background = "white";
            },2000);
      CAL.get("title-cal-edit").innerHTML = "l'heure est obligatoire";
      return;
  }
 /* if ((CAL.get("radio_call").checked == true) &&  (CAL.get("parent_id").value = "")){
      CAL.enable_buttons();
      CAL.get("title-cal-edit").innerHTML = "Please enter related field";
      return;
  }*/
        ...
        success: function (o) {
            ...
            if (res.access == 'yes') {
                ...
                if (undefined !== global_colorList[res.module_name]) {
                    if((res.status=='Held') && (res.module_name=='Calls')){
                        newEvent.backgroundColor = '#1236FF';
                        newEvent.borderColor = '#' + global_colorList[res.module_name].border;
                        newEvent.textColor = '#' + global_colorList[res.module_name].text;
                    }
                    else if((res.status=='Held') && (res.module_name=='Metings')){
                        newEvent.backgroundColor = '#1710FF';
                        newEvent.borderColor = '#' + global_colorList[res.module_name].border;
                        newEvent.textColor = '#' + global_colorList[res.module_name].text;
                    }
                    else{
    ...
    console.log(CAL.get("CalendarEditView"))
    YAHOO.util.Connect.asyncRequest('POST', url, callback, false);
}
...
$($.fullCalendar).ready(function () {
    ...
    $.each(calendar_items, function (user_id, user_calendar_activities) {
        ...
        $.each(user_calendar_activities, function (index, element) {
            ...
            if((element.status=='Held') && (element.module_name=='Calls')){
                valueToPush["backgroundColor"] = '#1236FF';
            ...
            else if((element.status=='Held') && (element.module_name=='Metings')){
                    valueToPush["backgroundColor"] = '#1798FF';
                    valueToPush["borderColor"] = '#' + global_colorList[element.module_name].border;
                    valueToPush["textColor"] = '#' + global_colorList[element.module_name].text;
                }
        ...
        $('#calendar' + user_id).fullCalendar({
            ...
            eventClick: function (calEvent, jsEvent, view) {
                var callback = {
                    success: function (o) {
                        console.log(o);
                        $('.modal-cal-tasks-edit .modal-body .container-fluid').html(o.responseText);
                        $('.modal-cal-tasks-edit').modal('show');
                    }}
                if ((typeof calEvent.status !== 'undefined') && (calEvent.status === 'Held')) {
                    /* var url = 'index.php?module=Calendar&action=detailview&record='+calEvent.id;
                    var data = {"current_module": calEvent.module, "record": calEvent.id, "edit_all_recurrences": "edit_all_recurrences"};
                    YAHOO.util.Connect.asyncRequest('POST', url, callback, CAL.toURI(data));
                    console.log('sfetdsm')*/
                    console.log(calEvent);
                    $.ajax('index.php?module=Calendar&action=detailview&record='+calEvent.id+'&current_module='+calEvent.module)
                        .success(function (o) {
                            console.log(o)
                            $('.modal-calls-meeting-detail .modal-body .container-fluid').html(o);
                            $('.modal-calls-meeting-detail').modal('show');
            ...
            eventDrop: function (event, delta, revertFunc) {
                if (event.status!='Held' && event.module!=="Meetings"){
            ...
            eventResize: function (event, delta, revertFunc) {
                ...
                debugger;console.log(event.module);
                if (event.status!='Held' && event.module!=="Meetings"){
