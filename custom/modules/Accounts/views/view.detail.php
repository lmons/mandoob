...
function subp_nav(m,i,a,t,r){
    ...
    t.href = url;
		...
		$CyclePlanTargetId='';
		$this->bean->load_relationship('accounts_thin_cycleplantargets_pharma_1');
		foreach($this->bean->accounts_thin_cycleplantargets_pharma_1->getBeans() as $iTargets => $oTarget)
		{
			$oTarget->load_relationship('thin_cycleplans_thin_cycleplantargets_pharma_1');

			foreach($oTarget->thin_cycleplans_thin_cycleplantargets_pharma_1->getBeans() as $iCyclePlan => $oCyclePlan)
			{if($oCyclePlan->active=='1'){
				$CyclePlanTargetId.="'".$oTarget->id."',";
			}
			}
		}
		$_REQUEST['active_cycle_plan_pharma']=$CyclePlanTargetId;
