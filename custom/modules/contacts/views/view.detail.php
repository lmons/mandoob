...
function subp_nav(m,i,a,t,r){
    ...
    t.href = url;
		...
		$CyclePlanTargetId='';
		$this->bean->load_relationship('thin_cycleplantargets_contacts');
		foreach($this->bean->thin_cycleplantargets_contacts->getBeans() as $iTargets => $oTarget)
		{
			$oTarget->load_relationship('thin_cycleplans_thin_cycleplantargets');

			foreach($oTarget->thin_cycleplans_thin_cycleplantargets->getBeans() as $iCyclePlan => $oCyclePlan)
			{if($oCyclePlan->active=='1'){
				$CyclePlanTargetId.="'".$oTarget->id."',";
			}
			}
		}
		$_REQUEST['active_cycle_plan']=$CyclePlanTargetId;
